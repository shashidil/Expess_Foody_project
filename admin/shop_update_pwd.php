...
<head>
    <?php
        ...
        if(isset($_POST["rst_confirm"])){
            ...
            if($newpwd != $newcfpwd){
                ...
                $query = "SELECT password FROM admin WHERE a_id = {$s_id} LIMIT 0,1;";
                $result = $mysqli -> query($query);
                $row = $result -> fetch_array();
                if($oldpwd == $row["password"]){
                    $query = "UPDATE admin SET password = '{$newpwd}' WHERE a_id = {$s_id};";
                    $result = $mysqli -> query($query);
                    if($result){
    ...
    <title>Update shop password | Express Food Meal Restaurant</title>
</head>
